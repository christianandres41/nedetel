...
?>
      ...
      <select <?=$campo['validacion']?> class="form-control combo-select2" style="width: 50%" id="<?=$c?>" name="<?=$c?>" data-campo-etiqueta-fk="<?=$__campo_etiqueta_fk?>" data-campo-etiqueta2-fk="<?=$__campo_etiqueta2_fk?>" <?=($c=="provincia")?"onchange='p_cargar_cantones_ciudades(this,0)'":(($c=="canton")?"onchange='p_cargar_parroquias(this,0)'":"onchange=''")?> >
        <option value="">&nbsp;</option>
<?php
/*
...
$('#<?=$c?>').select2({
  ...
  <script>
    function p_cargar_cantones_ciudades(target, cae_id){
        ...
        $('#canton').html('<option value="">Escoja primero la provincia</option>');
        //$('#nod_ciudad').prop('disabled', true);
        //$('#nod_ciudad').html('<option value="">Escoja primero la provincia</option>');
        $('#representante_legal_parroquia').prop('disabled', true);
        $('#representante_legal_parroquia').html('<option value="">Escoja primero el cantón</option>');
    ...
    function p_cargar_parroquias(target, cae_id){
        ...
        if (can_id) {
             ...
             */
            $('#representante_legal_parroquia').prop('disabled', false);
            $('#representante_legal_parroquia').html('<option value="">&nbsp;</option>');
            $('#representante_legal_parroquia').select2({
                language: "es"
                ,width: '100%'
                ,ajax: {
                    url: function (params) {
...
<div id="modal_historial" class="modal fade" tabindex="-1" role="dialog">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-nuevo-title">Historial de <?=isset($titulo_proceso_singular)?$titulo_proceso_singular:'cambios'?> <span id="historial_titulo"></span></h4>
      </div>
      <div class="modal-body">
...
function p_abrir_historico_precio_desde_detalle(){
    $('#modal').modal('hide');
...
function p_toggle_historico_precio(prc_id, prc_nombre){
    ...
    $.get('/_obtenerValoresHistoricosPrecios/' + prc_id, function(data){
        console.log('/_obtenerValoresHistoricosPrecios/' + prc_id, data);
        data = JSON.parse(data);
        console.log('data', data);
